...
namespace ImGui
{
    ...
    //    BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding BeginXXX function
    //    returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
    IMGUI_API bool          BeginChild(const char* str_id, const ImVec2& size, bool* child_active, ImGuiWindowFlags extra_flags = 0, const char* icon = "");
    IMGUI_API bool          BeginChild(ImGuiID id, const ImVec2& size, bool *border, ImGuiWindowFlags flags = 0);
    IMGUI_API void          EndChild(const char* name = "", bool child_active = true);
    ...
    void                    MultiCombo(const char* label, bool combos[], const char* items[], int items_count = 4);
    // Widgets: Main
    ...
    IMGUI_API bool          CheckboxColor(const char* label, bool* v, ImColor color);
    IMGUI_API bool          Checkbox(const char* label, bool* v, bool settings = false);
    IMGUI_API bool          Keybind(const char* label, int* key);
    IMGUI_API bool          SubTab(const char* label, int* v, int number);
    IMGUI_API bool          Tab(const char* label, const char* icon, int* v, int number);
    ...
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags = 0, int value = 5);
