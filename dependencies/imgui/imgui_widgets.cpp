...
bool ImGui::ScrollbarEx( const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags ) {
    ...
    window->DrawList->AddRectFilled( grab_rect.Min, grab_rect.Max, stroke_color, style.ScrollbarRounding );
...
void ImGui::MultiCombo( const char* label, bool combos[ ], const char* items[ ], int items_count ) {
    ...
    for ( int i = 0, j = 0; i < items_count; i++ ) {
        if ( combos[ i ] ) {
            ...
            if ( j > 2 )
                preview = vec.at( 0 ) + ", " + vec.at( 1 ) + ", " + vec.at( 2 ) + "," + vec.at( 3 );
            else if ( j )
    ...
    std::string trimmed = preview.substr( 0, 12 ) + "...";

    if ( BeginCombo( label, preview.length( ) > 12 ? trimmed.c_str( ) : preview.c_str( ) ) ) {
...
bool ImGui::Keybind( const char* label, int* key ) {
    ...
    it_anim->second.text_color = ImLerp( it_anim->second.text_color, it_anim->second.mode ? text_color_active : text_color_default, anim_speed );
    it_anim->second.key_color = ImLerp( it_anim->second.key_color, it_anim->second.mode ? main_color : text_color_default, anim_speed );
    ...
    window->DrawList->AddText( ImVec2( total_bb.Min.x, center_text( total_bb.Min, total_bb.Max, label ).y ), GetColorU32( it_anim->second.text_color ), label );

    char keyHex[ 16 ];
    if ( *key > 0 && !it_anim->second.mode ) {
        sprintf_s( keyHex, sizeof( keyHex ), "0x%02X", *key );
        it_anim->second.key_size = ImLerp( it_anim->second.key_size, CalcTextSize( keyHex ), anim_speed );
        window->DrawList->AddText( ImVec2( total_bb.Max.x - it_anim->second.key_size.x, center_text( total_bb.Min, total_bb.Max, keyHex ).y ), GetColorU32( it_anim->second.key_color ), keyHex );
    }
    ...
    if ( *key == 0 || it_anim->second.mode ) {
        ...
        window->DrawList->AddText( ImVec2( total_bb.Max.x - it_anim->second.key_size.x, center_text( total_bb.Min, total_bb.Max, "None" ).y ), GetColorU32( it_anim->second.key_color ), "None" );
    }

    return pressed;
}
struct checkboxcolor_state {
    float circle_offset;
    ImVec4 rect_color;
    ImVec4 text_color;
...
bool ImGui::CheckboxColor( const char* label, bool* v, ImColor color ) {
    ImGuiWindow* window = GetCurrentWindow( );
    if ( window->SkipItems )
        return false;
    ...
    const ImRect settings_bb( total_bb.Max - ImVec2( 35, 13 ), total_bb.Max - ImVec2( 20, 0 ) );

    const ImRect check_bb( total_bb.Max - ImVec2( 13, 13 ), total_bb.Max );
    ItemSize( total_bb, style.FramePadding.y );
    ItemAdd( total_bb, id );

    static std::map<ImGuiID, checkboxcolor_state> anim;
    auto it_anim = anim.find( id );

    if ( it_anim == anim.end( ) ) {
        anim.insert( { id, checkboxcolor_state( ) } );
        it_anim = anim.find( id );
    }


    bool hovered_settings = IsMouseHoveringRect( settings_bb.Min, settings_bb.Max );
    ...
    if ( pressed ) {
        if ( !hovered_settings ) {
            *v = !( *v );
            ...
            settings_win_pos = ImGui::GetMousePos( ) - win_pos;
            it_anim->second.settings = !it_anim->second.settings;
        }
    }

    RenderNavHighlight( total_bb, id );

    it_anim->second.text_color = ImLerp( it_anim->second.text_color, *v ? text_color_active : text_color_default, anim_speed );
    it_anim->second.rect_color = ImLerp( it_anim->second.rect_color, *v ? GetColorWithAlpha( color, 0.4f ) : ImColor( 0.f, 0.f, 0.f, 0.f ), anim_speed );
    it_anim->second.mark_rotate = ImLerp( it_anim->second.mark_rotate, *v ? IM_PI / 2 : IM_PI * 2, anim_speed );
    it_anim->second.mark_color = ImLerp( it_anim->second.mark_color, *v ? color : ImColor( 0.f, 0.f, 0.f, 0.f ), anim_speed );
    it_anim->second.settings_col = ImLerp( it_anim->second.settings_col, hovered_settings ? main_color : ImColor( 1.f, 1.f, 1.f, 0.25f ), anim_speed );

    window->DrawList->AddText( ImVec2( total_bb.Min.x, center_text( total_bb.Min, total_bb.Max, label ).y ), GetColorU32( it_anim->second.text_color ), label );

    window->DrawList->AddRectFilled( check_bb.Min, check_bb.Max, GetColorU32( it_anim->second.rect_color ), 2.5f );
    window->DrawList->AddRect( check_bb.Min, check_bb.Max, color, 2, 0, 1.3f );

    PushFont( icon_font_small ); ImRotateStart( );
    window->DrawList->AddText( center_text( check_bb.Min, check_bb.Max + ImVec2( 1, 0 ), "f" ), GetColorU32( it_anim->second.mark_color ), "f" );
    PopFont( ); ImRotateEnd( it_anim->second.mark_rotate );

    return  it_anim->second.settings;
}
...
bool ImGui::Checkbox( const char* label, bool* v, bool settings ) {
    ImGuiWindow* window = GetCurrentWindow( );
    if ( window->SkipItems )
        return false;
    ...
    it_anim->second.text_color = ImLerp( it_anim->second.text_color, *v ? text_color_active : text_color_default, anim_speed );
    it_anim->second.rect_color = ImLerp( it_anim->second.rect_color, *v ? main_color : ImColor( 0.f, 0.f, 0.f, 0.f ), anim_speed );
    it_anim->second.mark_rotate = ImLerp( it_anim->second.mark_rotate, *v ? IM_PI / 2 : IM_PI * 2, anim_speed );
    it_anim->second.mark_color = ImLerp( it_anim->second.mark_color, *v ? text_color_active : ImColor( 0.f, 0.f, 0.f, 0.f ), anim_speed );
    it_anim->second.settings_col = ImLerp( it_anim->second.settings_col, hovered_settings ? main_color : ImColor( 1.f, 1.f, 1.f, 0.25f ), anim_speed );

    window->DrawList->AddText( ImVec2( total_bb.Min.x, center_text( total_bb.Min, total_bb.Max, label ).y ), GetColorU32( it_anim->second.text_color ), label );

    window->DrawList->AddRectFilled( check_bb.Min, check_bb.Max, GetColorU32( it_anim->second.rect_color ), 2.5f );
    window->DrawList->AddRect( check_bb.Min, check_bb.Max, stroke_color, 2, 0, 1.3f );

    if ( settings ) {
        PushFont( icon_font_regular );
        window->DrawList->AddText( center_text( settings_bb.Min, settings_bb.Max + ImVec2( 1, -1 ), "g" ), GetColorU32( it_anim->second.settings_col ), "g" );
        PopFont( );
    }
...
bool ImGui::SubTab( const char* label, int* v, int number ) {
    ImGuiWindow* window = GetCurrentWindow( );
    if ( window->SkipItems )
        return false;
    ...
    it_anim->second.frame_col = ImLerp( it_anim->second.frame_col, *v == number ? ImColor( 1.f, 1.f, 1.f, 0.03f ) : ImColor( 0, 0, 0, 0 ), anim_speed );
    it_anim->second.text_col = ImLerp( it_anim->second.text_col, *v == number ? text_color_active : text_color_default, anim_speed );
    it_anim->second.line_col = ImLerp( it_anim->second.line_col, *v == number ? main_color : ImColor( 0, 0, 0, 0 ), anim_speed );

    window->DrawList->AddRectFilled( total_bb.Min, total_bb.Max, GetColorU32( it_anim->second.frame_col ), style.FrameRounding );
    window->DrawList->AddText( center_text( total_bb.Min, total_bb.Max, label ), GetColorU32( it_anim->second.text_col ), label );
    window->DrawList->AddRectFilled( total_bb.Min + ImVec2( 27, 38 ), total_bb.Max - ImVec2( 27, 0 ), GetColorU32( it_anim->second.line_col ), 360.f, ImDrawFlags_RoundCornersTop );
...
bool ImGui::Tab( const char* label, const char* icon, int* v, int number ) {
    ImGuiWindow* window = GetCurrentWindow( );
    if ( window->SkipItems )
        return false;
    ...
    const float square_sz = GetFrameHeight( );
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb( pos, pos + ImVec2( 168, 40 ) );
    ...
    it_anim->second.frame_col = ImLerp( it_anim->second.frame_col, *v == number ? ImColor( 1.f, 1.f, 1.f, 0.03f ) : ImColor( 0, 0, 0, 0 ), anim_speed );
    it_anim->second.text_col = ImLerp( it_anim->second.text_col, *v == number ? text_color_active : text_color_default, anim_speed );
    it_anim->second.icon_col = ImLerp( it_anim->second.icon_col, *v == number ? main_color : text_color_default, anim_speed );
    it_anim->second.line_col = ImLerp( it_anim->second.line_col, *v == number ? main_color : ImColor( 0, 0, 0, 0 ), anim_speed );

    window->DrawList->AddRectFilled( total_bb.Min, total_bb.Max, GetColorU32( it_anim->second.frame_col ), style.FrameRounding );
    PushFont( icon_font_big );
    window->DrawList->AddText( ImVec2( total_bb.Min.x + 15.f, center_text( total_bb.Min, total_bb.Max, label ).y ), GetColorU32( it_anim->second.icon_col ), icon );
    PopFont( );
    window->DrawList->AddText( ImVec2( total_bb.Min.x + 48, center_text( total_bb.Min, total_bb.Max, label ).y ), GetColorU32( it_anim->second.text_col ), label );
    window->DrawList->AddRectFilled( total_bb.Min + ImVec2( 0, 10 ), total_bb.Max - ImVec2( 166, 10 ), GetColorU32( it_anim->second.line_col ), 360.f, ImDrawFlags_RoundCornersRight );
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags, int value ) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow( );
    ...
    const ImRect total_bb( pos, pos + frame_size );
    const ImRect rect_bb( total_bb.Min + ImVec2( 100, -1 ), total_bb.Max + ImVec2( 0, 1 ) );
    ItemSize( total_bb, style.FramePadding.y );
    ItemAdd( total_bb, id );
    ...
    it_anim->second.combo_size = ImLerp( it_anim->second.combo_size, it_anim->second.opened_combo ? value * 26 + 10 : 0.f, anim_speed );
    it_anim->second.text_color = ImLerp( it_anim->second.text_color, it_anim->second.opened_combo ? text_color_active : text_color_default, anim_speed );

    window->DrawList->AddRectFilled( rect_bb.Min, rect_bb.Max, ImColor( 1.f, 1.f, 1.f, 0.05f ), style.FrameRounding );

    window->DrawList->AddText( ImVec2( total_bb.Min.x, center_text( total_bb.Min, total_bb.Max, label ).y ), GetColorU32( it_anim->second.text_color ), label );
    ImVec2 label_size = CalcTextSize( label );

    PushFont( medium_small_font );
    window->DrawList->AddText( ImVec2( rect_bb.Min.x + 4, center_text( rect_bb.Min, rect_bb.Max, preview_value ).y ), ImColor( 1.f, 1.f, 1.f, 0.25f ), preview_value );
    PopFont( );

    PushFont( icon_font_medium );
    window->DrawList->AddText( center_text( rect_bb.Max - ImVec2( 15, 15 ), rect_bb.Max, "0" ) - ImVec2( 1.5, 0 ), ImColor( 1.f, 1.f, 1.f, 0.25f ), "0" );
    PopFont( );

    /*
    PushFont(icon_font);
    window->DrawList->AddText(ImVec2(rect_bb.Max.x - CalcTextSize("1").x - 5, center_text(rect_bb.Min, rect_bb.Max, "1").y), main_color, it_anim->second.opened_combo ? "6" : "1");
    PopFont();
    */

    it_anim->second.text_size = ImLerp( it_anim->second.text_size, CalcTextSize( preview_value ), anim_speed );
...
int lerp_void( void* a, void* b, float t ) {
    // ���������� ���������� � ���� float*
    float* a_float = static_cast< float* >( a );
    float* b_float = static_cast< float* >( b );

    // ���������� �������� ������������
    float result = *a_float + t * ( *b_float - *a_float );
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags ) {
    ...
    static int p_data2 = 2;

    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior( ImRect( frame_bb.Min + ImVec2( 91, 2 ), frame_bb.Max + ImVec2( 7, frame_bb.Max.y - 2 ) ), id, data_type, p_data, p_min, p_max, format, flags, &grab_bb );
    if ( value_changed )
        MarkItemEdited( id );


    if ( hovered && ImGui::IsMouseDown( ImGuiMouseButton_Left ) ) {
        it_anim->second.value_changing = true;
    }

    if ( it_anim->second.value_changing && !ImGui::IsMouseDown( ImGuiMouseButton_Left ) )
        it_anim->second.value_changing = false;


    it_anim->second.grab_offset = ImLerp( it_anim->second.grab_offset, grab_bb.Min.x + ( grab_bb.Max.x - grab_bb.Min.x ) / 2 - frame_bb.Min.x, anim_speed * 5 );
    it_anim->second.text_color = ImLerp( it_anim->second.text_color, it_anim->second.value_changing ? text_color_active : text_color_default, anim_speed );
    it_anim->second.value_color = ImLerp( it_anim->second.value_color, it_anim->second.value_changing || hovered ? text_color_active : ImColor( 1.f, 1.f, 1.f, 0.f ), anim_speed * 2 );

    PushClipRect( frame_bb.Min + ImVec2( 100, 0 ), frame_bb.Max + ImVec2( 10, 0 ), true );
    window->DrawList->AddRectFilled( frame_bb.Min + ImVec2( 100, 3 ), ImVec2( frame_bb.Min.x + it_anim->second.grab_offset, frame_bb.Max.y - 3 ), main_color, style.FrameRounding );
    PopClipRect( );

    char value_buf[ 64 ];
    const char* value_buf_end = value_buf + DataTypeFormatString( value_buf, IM_ARRAYSIZE( value_buf ), data_type, p_data, format );

    it_anim->second.text_offset = ImLerp( it_anim->second.text_offset, it_anim->second.value_changing || hovered ? CalcTextSize( value_buf ).y / 2 : 0.f, anim_speed );

    window->DrawList->AddRect( frame_bb.Min + ImVec2( 100, 3 ), frame_bb.Max - ImVec2( 0, 3 ), stroke_color, style.FrameRounding, 0, 1.3f );
    window->DrawList->AddText( ImVec2( frame_bb.Min.x, center_text( frame_bb.Min, frame_bb.Max, label ).y ), GetColorU32( it_anim->second.text_color ), label );

    PushFont( medium_small_font );
    window->DrawList->AddText( center_text( frame_bb.Min + ImVec2( 100, 3 ), frame_bb.Max, value_buf ) + ImVec2( 0, it_anim->second.text_offset ), GetColorU32( it_anim->second.value_color ), value_buf );
    PopFont( );
...
bool ImGui::InputTextEx( const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data ) {
    ...
    if ( is_multiline ) {
        ...
        static bool child_active = false;
        bool child_visible = BeginChildEx( label, id, frame_bb.GetSize( ), &child_active, ImGuiWindowFlags_NoMove );
        PopStyleVar( 3 );
        PopStyleColor( );
        if ( !child_visible ) {
            EndChild( );
    ...
    if ( !is_multiline ) {
        RenderNavHighlight( frame_bb, id );
        window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, stroke_color, style.FrameRounding );
    }
...
void ImGui::DebugNodeInputTextState( ImGuiInputTextState* state ) {
    ...
    if ( BeginChild( "undopoints", ImVec2( 0.0f, GetTextLineHeight( ) * 15 ), &child_active ) ) // Visualize undo state
    {
...
bool ImGui::ColorEdit4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags ) {
    ...
    if ( !( flags & ImGuiColorEditFlags_NoSmallPreview ) ) {
        ...
        ImGui::SetCursorScreenPos( total_bb.Max - ImVec2( 13, 13 ) );
        ...
        PushStyleColor( ImGuiCol_Border, ImColorToImVec4( child_stroke_color ) );
    ...
    it_anim->second.color = ImLerp( it_anim->second.color, picker_active_window == NULL ? text_color_default : text_color_active, anim_speed );
    ...
    if ( label != label_display_end && !( flags & ImGuiColorEditFlags_NoLabel ) ) {
        ...
        window->DrawList->AddText( ImVec2( total_bb.Min.x, center_text( total_bb.Min, total_bb.Max, label ).y ), GetColorU32( it_anim->second.color ), label );
    }
...
bool ImGui::ColorPicker4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags, const float* ref_col ) {
    ...
    if ( it_anim == anim.end( ) ) {
        anim.insert( { GetID( label ), picker_state( ) } );
        it_anim = anim.find( GetID( label ) );
    }
    ...
    if ( flags & ImGuiColorEditFlags_PickerHueWheel ) {
        ...
        it_anim->second.sv_cursor_pos.x = ImLerp( it_anim->second.sv_cursor_pos.x, ImClamp( IM_ROUND( picker_pos.x + ImSaturate( S ) * sv_picker_size ), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2 ), g.IO.DeltaTime * 30.f ); // Sneakily prevent the circle to stick out too much
        it_anim->second.sv_cursor_pos.y = ImLerp( it_anim->second.sv_cursor_pos.y, ImClamp( IM_ROUND( picker_pos.y + ImSaturate( 1 - V ) * sv_picker_size ), picker_pos.y + 2, picker_pos.y + sv_picker_size - 2 ), g.IO.DeltaTime * 30.f );
    ...
    it_anim->second.sv_cursor_rad = ImLerp( it_anim->second.sv_cursor_rad, value_changed_sv ? 6.0f : 3.0f, g.IO.DeltaTime * 12.f );
...
bool ImGui::ColorButton( const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg ) {
    ...
    const float default_size = GetFrameHeight( );
    const ImVec2 size( frame_size.y, frame_size.y );
    const ImRect bb( window->DC.CursorPos, window->DC.CursorPos + size );
    ...
    window->DrawList->AddCircleFilled( bb.Min + size / 2, size.x / 2, GetColorU32( col_source ), 360 );
    window->DrawList->AddCircle( bb.Min + size / 2, size.x / 2, stroke_color, 360, 1.3f );
...
bool ImGui::Selectable( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg ) {
    ...
    it_anim->second.color = ImLerp( it_anim->second.color, selected ? main_color : text_color_active, anim_speed );
    it_anim->second.frame_color = ImLerp( it_anim->second.frame_color, hovered ? stroke_color : ImColor( 0, 0, 0, 0 ), anim_speed );

    std::cout << size_arg.y << std::endl;

    window->DrawList->AddRectFilled( bb.Min, bb.Max, GetColorU32( it_anim->second.frame_color ), style.FrameRounding );
    window->DrawList->AddText( text_min, GetColorU32( it_anim->second.color ), label );
